# cuanto vale cada voto en su parte decimal, *100 y sacas parte entera
#
...
def solve(npersons, nlanguages, langvalues_original):
    ...
    langvalues = [x*votevalue for x in (langvalues_original + [0]*(npersons - sum(langvalues_original)))]

    missing_people = []

    langvalues_decimals = list(map(lambda lang: lang - math.floor(lang), langvalues))
    for idx, lang in enumerate(langvalues_decimals):
        if lang < 0.5:
            accum += math.floor(langvalues[idx])
            missing_people.append((math.ceil((0.5-lang)/decimal_votevalue), lang))
        else:
            accum += math.ceil(langvalues[idx])
            missing_people.append((math.ceil((1.5-lang)/decimal_votevalue), lang))
    ...
    while remaining > 0:
        if remaining >= missing_people_ordered[0][0]:
            ...
            accum += 1
            nueva_suma = missing_people_ordered[1][0] + missing_people_ordered[0][0]*decimal_votevalue
            nueva_suma = nueva_suma - math.floor(nueva_suma)
            accum += math.floor(missing_people_ordered[0][0]*votevalue)
            if nueva_suma < 0.5:
                nuevaTupla = (math.ceil((0.5-nueva_suma)/decimal_votevalue), nueva_suma)

            else:
                nuevaTupla = (math.ceil((1.5-lang)/decimal_votevalue), nueva_suma)
